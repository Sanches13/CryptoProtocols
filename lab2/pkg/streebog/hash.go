...
func (h *Hash) Close() {
	...
	log.Println("Очищена память, хранящая ключевую информацию в структуре Hash")
	runtime.GC()
}
...
func g(n uint64, hsh, data *[BlockSize]byte) *[BlockSize]byte {
	...
	copy(r[8:], hsh[8:])
	return xor(xor(e(l(ps(r)), data), hsh), data)
}
...
func xor(x, y *[BlockSize]byte) *[BlockSize]byte {
	r := new([BlockSize]byte)
	for i := 0; i < BlockSize; i++ {
		r[i] = x[i] ^ y[i]
	}
